...
remote_file "/tmp/go-#{version}.tar.gz" do
  source "http://go.googlecode.com/files/go#{version}.linux-#{arch}.tar.gz"
  mode "0644"
  checksum node[:golang][version][arch][:checksum]
  not_if { ::File.exists?("/tmp/go-#{version}.tar.gz") }
end

execute "tar -C #{install_location} --strip-components=1 -xzf /tmp/go-#{version}.tar.gz" do
